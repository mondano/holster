...
describe("holster.user.lex", () => {
  ...
  test("put and get object in graph format", async () => {
    ...
    const data1 = await new Promise(resolve => user.get("plain", { ".": { "*": "k" } }, resolve))
    expect(data1).toEqual({ key: "plain value" })

    const data2 = await new Promise(resolve => user.get("plain", { ".": { "*": "t" } }, resolve))
    expect(data2).toEqual({ true: true })

    const data3 = await new Promise(resolve => user.get("plain", { ".": { "*": "f" } }, resolve))
    expect(data3).toEqual({ false: false })

    const data4 = await new Promise(resolve => user.get("plain", { ".": { "*": "n" } }, resolve))
    expect(data4).toEqual({ number: 42 })

    // Both less than and greater than.
    const data5 = await new Promise(resolve =>
      user.get("plain", { ".": { "<": "n", ">": "falsy" } }, resolve)
    )
    expect(data5).toEqual({ key: "plain value" })

    // Only less than.
    const data6 = await new Promise(resolve => user.get("plain", { ".": { "<": "k" } }, resolve))
    expect(data6).toEqual({ false: false })

    // Only greater than.
    const data7 = await new Promise(resolve =>
      user.get("plain", { ".": { ">": "numbers" } }, resolve)
    )
  ...
  test("on and put nested object", async () => {
    ...
    const err = await new Promise(resolve => user.get("on-nested").put(nested, resolve))
    expect(err).toBe(null)
    ...
    const err2 = await new Promise(resolve => user.get("on-nested").put(update, resolve))
  ...
  test("cleanup", async () => {
    await fs.promises.rm("test/holster.user.lex", { recursive: true, force: true })
  })
})
